...
contract Domains is ERC721URIStorage {
    ...
    mapping(string => address) public domains; // xx.lc, xx.a16z, xx.ninja
    mapping(string => string) public favouriteFood;
    mapping(uint => string) public domainNames; // shiqi.domain, ogb.domain

    error Unauthorized();
    error AlreadyRegistered();
    error InvalidName(string name);

    constructor(string memory _tld) payable ERC721("LionCity Name Service", "LCNS"){
        topLevelDomain = _tld;
        owner = payable(msg.sender);
        console.log("'%s' name service is now live", _tld);
    }
    ...
    function register(string calldata name) public payable {
        ...
        console.log("Registering %s on the contract with tokenID %d!", name, nftID);
    ...
    function setFavouriteFood(string calldata name, string calldata food) public {
        if (msg.sender != domains[name]) revert Unauthorized();
        favouriteFood[name] = food;
    }
    
    function getFavouriteFood(string calldata name) public view returns (string memory) {
        return favouriteFood[name];
    }
    ...
    function withdraw() public onlyOwner {
        ...
        require(success, "Failed to withdraw MATIC, you are not the owner of the contract");
    }
